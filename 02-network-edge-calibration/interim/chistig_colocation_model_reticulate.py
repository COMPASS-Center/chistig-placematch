...
class Ego(core.Agent):
    ...
    def __init__(self, ego_id: int, rank: int):
        ...
        self.epimodeluid = 's' + str(ego_id).zfill(5)
        self.epimodelpid = 's' + str(ego_id).zfill(5)
    ...
    def save(self) -> Tuple:
        ...
        return (self.uid,
                ...
                self.epimodeluid, self.epimodelpid)
...
def restore_agent(agent_data: Tuple):
    ...
    sego.epimodeluid = agent_data[11]
    sego.epimodelpid = agent_data[12]    
    return sego
...
class Model:
    ...
    def __init__(self, comm: MPI.Intracomm, params: Dict):
        ...
        self.agent_logger = logging.TabularLogger(comm, params['agent.log.file'], ['tick', 'agent_id', 'agent_uid_rank', 'ego_id', 'age', 'age_group', 'race_ethnicity', 'hiv_status', 'relationship_status', 'assigned_eego', 'venues_attended', 'apps_used', 'epimodel_uid', 'epimodel_pid'])
        ...
        for index, row in segodf.iterrows():
            ...
            sego.epimodeluid = row['egoid']
            sego.epimodelpid = row['egoid']
    ...
    def log_agents(self):
        ...
        if tick >= params['burnin.time.weeks']: 
            for sego in self.context.agents():
                self.agent_logger.log_row(tick, sego.id, sego.uid_rank, sego.egoid, sego.age, sego.agegroup, sego.raceethnicity, sego.hivstatus, sego.relationshipstatus, sego.eego, sego.venues_attended, sego.apps_used, sego.epimodeluid, sego.epimodelpid)
    ...
    def add_agent_from_epimodel(self, agentid, agentegoid, agentraceethnicity, agentdemocode):
        ...
        sego.epimodeluid = agentegoid
        sego.epimodelpid = agentegoid        
        #
        ######################
    ...
    def update_egos_from_epimodel(self, activeegoslist, segos2epimodeluiddict, segos2epimodelpiddict, segos2egoiddict, segos2reldict, segos2hivdict, segos2agedict):
        egos2remove = []
        for sego in self.context.agents(Ego.TYPE):
            segonumericid = int(sego.id)
            if segonumericid in activeegoslist:
                assert(sego.egoid == segos2egoiddict[segonumericid])
                # set/update epimodel uid/pid values
                sego.epimodeluid = segos2epimodeluiddict[segonumericid]
                sego.epimodelpid = segos2epimodelpiddict[segonumericid]
...
def update_egos(segosdf):
    ...
    segosdf['EpiModel_uid'] = segosdf['EpiModel_uid'].astype(int)
    segosdf['EpiModel_pid'] = segosdf['EpiModel_pid'].astype(int)    

    active_egos = segosdf['numeric.id'].to_list()
    segos2epimodeluid_dict = segosdf.set_index('numeric.id')['EpiModel_uid'].to_dict()
    segos2epimodelpid_dict = segosdf.set_index('numeric.id')['EpiModel_pid'].to_dict()
    ...
    model.update_egos_from_epimodel(active_egos, segos2epimodeluid_dict, segos2epimodelpid_dict, segos2egoid_dict, segos2rel_dict, segos2hiv_dict, segos2age_dict)
