...
#SBATCH --mem=3G
#SBATCH --job-name="venues_onetime_test" ## When you run squeue -u NETID this is how you can identify the job
#SBATCH --output=venues_onetime_test.out ## standard out and standard error goes to this file
...
IFS=$'\n' read -d '' -r -a input_args < step2_ergm_fit_procedure_input_args.txt
...
echo "Run number in SLURM array: XX" 
echo "YAML file name: 02-edge-calibration.yaml."
echo "Calibration set number for ERGM fit: 9." 
echo "Treatment type for ERGM fit: venues."
echo "Partnership type for ERGM fit: onetime."
...
for attempt in "${!my_random_seed_array[@]}"; do
	...
	if timeout "$timeout_duration" Rscript step2b_edge_calibration.R 02-edge-calibration.yaml 9 venues onetime "${my_random_seed_array[$attempt]}"; then
