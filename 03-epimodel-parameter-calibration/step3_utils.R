...
# SARA NEEDS TO DO THE YAML INTEGRATION PART HERE IF WE'RE STILL DOING THIS
...
est_dir <- "./03-epimodel-parameter-calibration/data/intermediate/estimates/"
diag_dir <- "./03-epimodel-parameter-calibration/data/intermediate/diagnostics/"
calib_dir <- "./03-epimodel-parameter-calibration/data/intermediate/calibration/"
scenarios_dir <- "./03-epimodel-parameter-calibration/data/intermediate/scenarios/"
...
epistats <- readRDS("./03-epimodel-parameter-calibration/data/intermediate/estimates/epistats-local.rds")
...
param <- EpiModel::param.net(
  data.frame.params = readr::read_csv("./03-epimodel-parameter-calibration/data/input/params_chistig.csv"),
  netstats          = netstats,
  epistats          = epistats,
  prep.start        = Inf,
  riskh.start       = Inf
...
swf_configs_quest <- function(partition = partition,
                              account = account,
                              r_version = r_version,
                              conda_proj = conda_proj,
                              mail_user = mail_user) {
  ...
  # THIS IS WHAT'S IN THE CHISTIG HPC REPO, BUT DO WE NEED TO CHANGE
...
# Custom `swf_configs` function for Quest made by Adrien
...
hpc_configs <- swf_configs_quest(
  partition = partition, # ASK ABOUT THIS
  r_version = r_version,
  mail_user = mail_user
)
#
