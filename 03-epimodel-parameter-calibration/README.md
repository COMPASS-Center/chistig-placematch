...
As mentioned above, [a full explanation of the parameter calibration process and a tutorial on how to set it up on an HPC can be found here](https://epimodel.github.io/swfcalib/articles/swfcalib.html). In our repository, `step1a_generate_automated_calibration.R` contains the code found in this tutorial and is what should be edited to create new calibration attempts. Using RStudio on a local machine, we modify this script to specify which parameters we want to calibrate, the range of values that should be explored for each parameter, and the order of calibration "waves" in which parameter values should be explored. This script is also where we define the "benchmark" outcomes associated with each calibrated parameter and what the values of these benchmarks should be. For example, because the parameter `hiv.test.rate_1` (which determines the rate at which Black individuals get tested for HIV) shapes the outcome measures `cc.dx.B` (the percentage of Black individuals diagnosed with HIV), we know that calibration for `hiv.test.rate_1` is complete when the parameter results in the benchmark `cc.dx.B` value of approximately 0.55. Benchmark values used in our calibration process, along with their sources and clarifying information about how these values were calculated, are contained in the `chistig_benchmarks.csv` file.

It is worth mentioning that `step1a_generate_automated_calibration.R` sources the `utils_03.R` script. This script checks the `./03-epimodel-parameter-calibration/data/intermediate/estimates/` directory for all `netest`/ERGM fit files required to simulate networks during parameter calibration. If these files are absent, `utils_03.R` will copy the contents of `./02-network-edge-calibration/output/` (i.e. the `netest`/ERGM fit files we created during network edge calibration) to `./03-epimodel-parameter-calibration/data/intermediate/estimates/`. Note, however, that we have set up `./03-epimodel-parameter-calibration/data/intermediate/estimates/` to contain the `netest`/ERGM fit files we used for our own study.

After saving modifications to `step1a_generate_automated_calibration.R`, we push our updated script to our GitHub repository.
...
Returning to your local machine's instance of the GitHub repository, restart R and run the `step1a_generate_automated_calibration.R` script we modified in Step 1. Running this script creates a directory of shell (`.sh`) files that will be used to run the automated calibration process on the HPC. This directory will be located in the `workflows` folder found in the main project directory (`chistig-placematch`). The directory containing the shell files will be given the name specified in the `create_workflow` function in `step1a_generate_automated_calibration.R` (the default name in our repository is `auto_calib`).

**NOTE:** `step1a_generate_automated_calibration.R` will only create the directory of shell files if this directory does not already exist within the `workflows` folder. If you have previously run `step1a_generate_automated_calibration.R`, you will need to delete `chistig-placematch/workflows/auto_calib` (or whatever your directory is named) and its contents before running `step1a_generate_automated_calibration.R` again.
...
With `calib_object` in front of you, you will want to copy all calibrated parameter values found in `calib_object$state$default_proposal` to the `chistig_params.csv` file. With your calibrated epidemiological parameters in tow, you can now move on to finalizing calibration of the `trans.scale` parameters. For your convenience, we've compiled the above pieces of code into the `step1b_automated.calibration_results.R` script. Additionally, this script will automatically write the calibrated parameter values in `calib_object` to their respective places in the `chistig_params.csv` file.

### 11. Manual Calibration of `trans.scale` Parameters

To finalize calibration of `trans.scale` parameter values, we recommend you calibrate manually using the `step2a_manual_calibration.R` script. In the portion of this script that creates the `scenarios_df` tibble object, you can modify the code to create the set of possible `trans.scale` value combinations you would like to explore (lines 120-123). Once you have made this modification, run the whole `step2a_manual_calibration.R` script to generate a set of simulations for each combination of parameter values you have defined. The results of these simulations will be stored in the `./03-epimodel-parameter-calibration/data/intermediate/calibration` subdirectory.

The `step2b_manual_calibration_diagnostics.R` script takes the simulations generated in `step2a`, extracts the outcome measures associated with benchmark values from each simulation, and displays these measures using a series of visualizations. These visualizations also plot the benchmark values stored in `chistig_benchmarks.csv` and allow us to see how close each combination of `trans.scale` values came to creating the desired outcome. Based on these visualized results, users can determine which combinations of `trans.scale` values were most successful and use these combinations to narrow the range of `trans.scale` values to explore in another iteration of `step2a`. Users will likely need to move through multiple iterations of the `step2a` and `step2b` scripts before arriving at satisfactory `trans.scale` values.

Once final `trans.scale` values are discovered, they should be manually recorded in the `chistig_params.csv` file. This should mark the completion of the parameter calibration process, and `chistig_params.csv` can then be carried over into the final simulation stage of our workflow contained in the `chistig-placematch/04-chistig-model-simulation` directory. For easy reference and replication, the `chistig_params.csv` native to this repository contains the calibrated parameter values we used in our final simulations.
