...
#SBATCH --mem=3G
#SBATCH --job-name="step4_\${SLURM_ARRAY_TASK_ID}" ## When you run squeue -u NETID this is how you can identify the job
#SBATCH --output=step4.%A_%a.out ## standard out and standard error goes to this file
...
IFS=$'\n' read -d '' -r -a input_args < step4_input_args.txt
echo ${input_args[$SLURM_ARRAY_TASK_ID]}

SECONDS=0
Rscript step4_simtest.R ${input_args[$SLURM_ARRAY_TASK_ID]}
echo $SECONDS
